...
function checkTableName($shortTName, $type=false)
{
	...
	if ("config_stats_info" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("config_layers_info" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("config_layers_group" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
		$arr[]="config.stats_info";
		$arr[]="config.layers_info";
		$arr[]="config.layers_group";
	return $arr;
}
...
function GetUserPermissions($table="")
{
	...
	if($table=="config.stats_info")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="config.layers_info")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="config.layers_group")
	{
			$permissions =  "ADESPIM";
	}
